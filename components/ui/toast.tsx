...
const toastVariants = cva(
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-xl border p-5 pr-10 shadow-xl transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full backdrop-blur-sm',
  {
    variants: {
      variant: {
        default: 'border-primary/20 bg-gradient-to-br from-primary/5 via-background to-primary/10 text-foreground',
        success: 'border-emerald-500/30 bg-gradient-to-br from-emerald-500/5 via-background to-emerald-500/10 text-foreground',
        destructive:
          'border-destructive/30 bg-gradient-to-br from-destructive/5 via-background to-destructive/10 text-foreground',
        info: 'border-blue-500/30 bg-gradient-to-br from-blue-500/5 via-background to-blue-500/10 text-foreground',
      },
    },
    defaultVariants: {
      variant: 'default',
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      'absolute right-2 top-2 rounded-lg p-1.5 text-foreground/40 opacity-0 transition-all hover:text-foreground hover:bg-foreground/5 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-primary/30 group-hover:opacity-100',
      className,
    )}
    toast-close=""
    {...props}
